...
export class ProductCard {
  ...
  addToCart(product: any) {
    ...
    if (customerId) {
        ...
        next: (cart) => {
          if (cart.length > 0) {
            ...
            this.cartService.update(userCart.id!, userCart).subscribe({
              next: () => { this.toast.success('Added to cart!', 'top-left', 2000); this.cartDrawer.open(); },
              error: (e) => this.toast.danger(e?.error || 'Failed to add to cart', 'top-left', 3000),
            });
          } else {
            const newCart: CartM = {
      ...
      this.toast.success('Added to cart!', 'top-left', 2000);
